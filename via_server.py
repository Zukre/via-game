...
def calc_flow(p):
    """Месячный ЧИСТЫЙ поток = доход − расходы. Используется для АВТО-начисления
    зарплаты при проходе клетки «Зарплата».

    💵 БЕЗ РАССИНХРОНА (Ринат 30июл): клиентский calc() кладёт свой точный поток в p['_flow']
    (ферма-расходы, карьерные ступени, «дно», дорожающая жизнь — всё уже учтено ровно как на
    экране игрока). Доверяем ему. Формула ниже — ЗАПАСКА для ботов и старых снимков без _flow."""
    _fv = p.get('_flow')
    if _fv is not None:
        try:
            return round(float(_fv), 2)
        except (TypeError, ValueError):
            pass
    prof = p.get('prof') or {}
...
SELF_TRAININGS = {
    ...
    'camera': {'name': 'Съёмка на камеру 🎥',       'cost': 2500},   # Ринат 28июл: открывает Стримера
...
CREATIONS = {
    ...
    # Ринат 28июл — 3 новые профессии:
    'streamer':   {'name': 'Стример 🎥',              'cost': 6000,  'cf': 700,
                   'req': ['camera'], 'boost': ['camera', 'brand', 'mkt', 'psy']},
    'standupact': {'name': 'Стендап 🎤',              'cost': 5000,  'cf': 600,
                   'req': ['standup'], 'boost': ['standup', 'stage', 'brand', 'psy']},
    'showroom':   {'name': 'Шоурум 🏬',               'cost': 20000, 'cf': 2500,
                   'req': ['bizproc', 'sales'], 'boost': ['bizproc', 'mkt', 'sales', 'nego']},
}
...
CREATION_STEPS = {
    ...
    'streamer':  {2: 'Донаты и спонсоры',   3: 'Своя студия и команда'},
    'standupact':{2: 'Сольные концерты',    3: 'Свой комедийный клуб'},
    'showroom':  {2: 'Второй шоурум',       3: 'Сеть шоурумов'},
}
